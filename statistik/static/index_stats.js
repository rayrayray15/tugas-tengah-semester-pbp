...
let init = () =>
{
	$.ajax(
	{
		...
		success: (data) => 
		{
			...
			overallCasesChart = new Chart(mainChart,
			{
				type: `line`,
				data: {
	...
	$.ajax(
	{
		...
		success: (data) => 
		{
			vaccineDemographicChart = new Chart(vaxDemographicChart,
			{
				type: `bar`,
				data: 
				{
			...
			vaccineStageChart = new Chart(vaxStageChart,
			{
				type: `bar`,
				data: 
				{
...
$(`#prov-select`).change(() =>
{
	if($(`#prov-select`).val() !== `Pilih provinsi`)
	{
